...
def load_all_companies():
    """Load all companies with enrichment data from v2 schema"""
    session = get_session()
    
    # Query all investments with company and PE firm data
    investments = session.query(CompanyPEInvestment).all()
    ...
    for inv in investments:
        ...
        data.append({
            'ID': c.id,
            'Company': c.name,
            'PE Firm': inv.pe_firm.name,
            'Sector': inv.sector_page or '',
            'Industry': c.swarm_industry or '',
            'Industry Category': c.industry_category or '',
            'Status': inv.computed_status or inv.raw_status or 'Unknown',
            'Founded': c.founded_year or '',
            'Investment Year': inv.investment_year or '',
            'Headquarters': c.headquarters or '',
            'Website': c.website or '',
            'LinkedIn': c.linkedin_url or '',
            'Description': c.description or '',
            'Exit Info': inv.exit_info or '',
            'Exit Type': inv.exit_type or '',
