...
class PortfolioCompany(Base):
    ...
    sector = Column(String(500), index=True)  # Scraper-specific sector
    ...
    sector_page = Column(String(255))  # PE-firm specific metadata
    ...
    employee_count = Column(String(50), index=True)  # Crunchbase employee range code
    ...
    # Categorization fields (computed from other fields)
    company_size_category = Column(String(50), index=True)  # Derived from employee_count: Startup/Small/Medium/Large/Enterprise
    revenue_tier = Column(String(50), index=True)  # Derived from revenue_range: Pre-Revenue/Early/Growth/Scale-up/Unicorn
    investment_stage = Column(String(50), index=True)  # Derived from investment_year: Recent/Mature/Legacy
    
    # Standardized industry classification
    industry_category = Column(String(100), index=True)  # Standardized industry category
    
    # Public company flag (useful for filtering)
